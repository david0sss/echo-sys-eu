...
start_idx = content.find("const translations: Record<string, Record<string, string>> = {")
end_idx = content.find("let activeLanguage = 'en';")

if start_idx != -1 and end_idx != -1:
    clean_dict = """const translations: Record<string, Record<string, string>> = {
  en: {
  ...
  nl: {
    ...
    content = content[:start_idx] + clean_dict + content[end_idx:]
    
    content = re.sub(r'export default App;\s*$', '', content)
    
    content = content.replace("const t = (key: string) => translations[lang][key as keyof typeof translations['en']] || translations['en'][key as keyof typeof translations['en']];", "")
